...
public sealed class ListTag : BaseTag
{
    ...
    public override void Write(BinaryWriter writer, ReadWriteOptions options, bool canHaveName = true)
    {
        ...
        for (int i = 0; i < Values.Count; i++)
        {
            ...
            NBT.WriteTag(writer, Values[i], payloadOptions, false);
        }
    }

    public static ListTag Read(BinaryReader reader, ReadWriteOptions options = default, bool canHaveName = true)
    {
        ...
        {
            BaseTag item = NBT.ReadTag(reader, elementType, payloadOptions, false);
            tag.Values.Add(item);
        }
