...
public abstract class NetworkConnection
{
  ...
  // Reusable buffers 
  private readonly List<uint> _pendingRemovalBuffer = [];
  private readonly List<Frame> _packedFramesBuffer = [];
  ...
  public virtual void HandleFrameSet(ReadOnlySpan<byte> message)
  {
    if (message.Length < 4)
    {
      return;
    ...
    FrameSetReader reader = FrameSet.CreateReader(message);
    ...
    while (reader.TryReadNext(out Frame frame))
    {
  ...
  public void HandleAck(Ack ack)
  {
    lock (_sendLock)
    {
      foreach (uint sequence in AckRecord.EnumerateRecords(ack.Records))
      {
  ...
  public void HandleNack(Nack nack)
  {
    lock (_sendLock)
    {
      foreach (uint sequence in AckRecord.EnumerateRecords(nack.Records))
      {
  ...
  public void Tick(long nowMs)
  {
    lock (_sendLock)
    {
      ...
      foreach ((uint sequence, PendingDatagram pending) in _pendingDatagrams)
      {
        if (nowMs - pending.SentAtMs >= AckResendMs)
        {
          _pendingRemovalBuffer.Add(sequence);
        }
      }

      for (int i = 0; i < _pendingRemovalBuffer.Count; i++)
      {
        uint sequence = _pendingRemovalBuffer[i];
        if (_pendingDatagrams.Remove(sequence, out PendingDatagram pending))
        {
          RequeueFrames(pending.Frames);
        }
  ...
  private void FlushAcks()
  {
    ...
    {
      int count = _ackQueue.Count;
      Span<uint> sequences = count <= 256
        ? stackalloc uint[count]
        : new uint[count];

      int idx = 0;
      foreach (uint seq in _ackQueue)
      {
        sequences[idx++] = seq;
      }

      Span<byte> buffer = stackalloc byte[ControlPacketBufferSize];
      int length = Ack.Serialize(Ack.FromSequences(sequences), buffer);
    ...
    {
      int count = _nackQueue.Count;
      Span<uint> sequences = count <= 256
        ? stackalloc uint[count]
        : new uint[count];

      int idx = 0;
      foreach (uint seq in _nackQueue)
      {
        sequences[idx++] = seq;
      }

      Span<byte> buffer = stackalloc byte[ControlPacketBufferSize];
      int length = Nack.Serialize(Nack.FromSequences(sequences), buffer);
      SendMessage(buffer[..length]);
      _nackQueue.Clear();
    }
  }
    ...
    bool immediate = false)
  {
    ...
    lock (_sendLock)
    {
      ...
      byte[] splitBuffer = ArrayPool<byte>.Shared.Rent(payload.Length);
      payload.CopyTo(splitBuffer);
      int offset = 0;
  ...
  private void FlushOutgoing(long nowMs)
  {
    ...
    int requiredBufferSize = Math.Max(ControlPacketBufferSize, MaxMtu * 2);

    if (_datagramBuffer is null || _datagramBuffer.Length < requiredBufferSize)
    {
      _datagramBuffer = new byte[requiredBufferSize];
    }

    while (_outgoingFrames.Count > 0)
    {
      _packedFramesBuffer.Clear();
      ...
      while (_outgoingFrames.Count > 0)
      {
        ...
        if (_packedFramesBuffer.Count > 0 && currentSize + frameSize > maxDatagramSize)
        {
          break;
        }

        if (currentSize + frameSize > _datagramBuffer.Length)
        {
          break;
        }

        _outgoingFrames.RemoveFirst();
        _packedFramesBuffer.Add(frame);
        currentSize += frameSize;
      }

      if (_packedFramesBuffer.Count == 0)
      {
        break;
      }

      uint sequence = _sendSequence++;
      int length = FrameSet.Serialize(sequence, _packedFramesBuffer, _datagramBuffer);

      bool reliable = false;
      for (int i = 0; i < _packedFramesBuffer.Count; i++)
      {
        if (NeedsReliableIndex(_packedFramesBuffer[i].Reliability))
        {
          reliable = true;
          break;
        }
      ...
      if (reliable)
      {
        _pendingDatagrams[sequence] = new PendingDatagram([.. _packedFramesBuffer], nowMs);
      }

      SendMessage(_datagramBuffer.AsSpan(0, length));
    }
  }
